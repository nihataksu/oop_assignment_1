...
class LassoRegressor(MultipleLinearRegressor):

    def __init__(self, default_intercept=0, default_coefficients=None, lasso_lambda=1.0):
        super().__init__(default_intercept, default_coefficients)
        self.lasso_lambda = lasso_lambda

    def _lasso_gradient(self, X, y, w):
        n = len(y)
        error = y - np.dot(X, w[1:]) - w[0]
        sign_vector = np.sign(w[1:])
        gradient = -2 / n * np.dot(X.T, error) + self.lasso_lambda * sign_vector
        return gradient
    ...
    def train(self, X: np.ndarray, y: np.ndarray, learning_rate=0.01, num_iterations=1000):
        try:
            ...
            for _ in range(num_iterations):
                gradient = self._lasso_gradient(X_with_intercept, y, w)
